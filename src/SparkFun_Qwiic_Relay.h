...
{
  uint8_t status = _readCommand(COMMAND_STATUS);
...
void Qwiic_Relay::_writeCommandOn(uint8_t _command)
{
  ...
  if( _command == RELAY_ONE ){
    uint8_t _status = readCommand(RELAY_ONE_STATUS);
    ...
    uint8_t _status = readCommand(RELAY_TWO_STATUS);
    if( _status == QUAD_RELAY_ON ){
      return;
    }
    else {
    ...
    uint8_t _status = readCommand(RELAY_THREE_STATUS);
    if( _status == QUAD_RELAY_ON ){
      return;
    }
    else {
    ...
    uint8_t _status = readCommand(RELAY_FOUR_STATUS);
    if( _status == QUAD_RELAY_ON ){
      return;
    }
    else {
...
uint8_t Qwiic_Relay::_readCommand(uint8_t _command)
{
  ...
  _i2cPort->beginTransmission(_address); 
  _i2cPort->write(command); // Moves pointer to register.
  _i2cPort->endTransmission();
...
float Qwiic_Relay::_readVersion(_command)
{
  _i2cPort->beginTransmission(_address); 
